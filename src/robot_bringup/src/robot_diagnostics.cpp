...
ros::Publisher robot_diagnostics_pub;

diagnostic_msgs::DiagnosticArray robot_diagnostics;
...
{
  setDiagnosisMsg(&imu_state, level, "IMU Sensor", message, "MPU9250");  // IMU 모델 확인 필요
}

void setMotorDiagnosis(uint8_t level, std::string message)
{
  setDiagnosisMsg(&motor_state, level, "Actuator", message, "DYNAMIXEL MX64");   
}
...
{
  setDiagnosisMsg(&LDS_state, level, "Depth Camra", message, "Realsens R200");
}
...
void msgPub()
{
  robot_diagnostics.header.stamp = ros::Time::now();

  robot_diagnostics.status.clear();
  robot_diagnostics.status.push_back(imu_state);
  robot_diagnostics.status.push_back(motor_state);
  robot_diagnostics.status.push_back(LDS_state);
  robot_diagnostics.status.push_back(battery_state);
  robot_diagnostics.status.push_back(button_state);

  robot_diagnostics_pub.publish(robot_diagnostics);
}
...
int main(int argc, char **argv)
{
  ...
  robot_diagnostics_pub  = nh.advertise<diagnostic_msgs::DiagnosticArray>("diagnostics", 10);
